...
import com.shuangtu.prison.discern.face.AutoFitTextureView;
import com.shuangtu.prison.discern.face.BoundingBoxView;
import com.shuangtu.prison.discern.face.DrawInfo;
import com.shuangtu.prison.discern.face.FaceEnv;
import com.shuangtu.prison.discern.face.http.common.HttpUtils;
...
public class CameraFragment extends Fragment {
    ...
    public CameraFragment(String url, int type) {
        this.url = url;
        this.type = type;
    }
...
//            }
    ...
    public void Init() {
        if (!inited) {
            inited = FaceEnv.init(this);
            //    HttpUtils.login(Constant.getLogin(), userName, password, loginType);
        }
    }


    public static CameraFragment newInstance(String url, int type) {
        return new CameraFragment(url, type);
    }
        ...
        try {
            ...
            root.post(new Runnable() {
                ...
                public void run() {
                    ...
                    try {
                        manager.openCamera(String.valueOf(0), mStateCallback, mBackgroundHandler);
                    } catch (CameraAccessException e) {
                        e.printStackTrace();
                    }
                }
    ...
    private class detectAsync extends AsyncTask<Bitmap, Void, DrawInfo> {
        private int index = 0;
        ...
        private String saveBitmap(Bitmap mBitmap) {
            String filePath = Environment.getExternalStorageDirectory() + "/face/tmp/";

            createDir(filePath);

            filePath += String.format("%06d", index++) + ".png";
        ...
        protected DrawInfo doInBackground(Bitmap... bp) {
            if (isRequest || isStop) {
                return new DrawInfo();
            }
            ...
            String filePath = FaceEnv.saveBitmap(getActivity(), bp[0]);

            if (filePath.length() > 0) {
                Rect rect = FaceEnv.FeatureEx(filePath, feature);
                if (rect.bottom > 0) {
                    result.setRect(rect);
                    Log.d(TAG, "extace face success. Rect;" + result);

                    UserFeatureVo userFeatureVo = HttpUtils.match(url, feature, type);

                    if (userFeatureVo != null) {
                        result.setUserFeatureVo(userFeatureVo);
                        isRequest = true;
                    } else {
                        isRequest = false;
                    }
        ...
        protected void onPostExecute(DrawInfo result) {
            mBoundingBoxView.setResults(result);
            mIsDetecting = false;
        }
    }
