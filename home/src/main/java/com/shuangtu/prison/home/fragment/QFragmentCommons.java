...
public abstract class QFragmentCommons extends QFragment {
    ...
    public void initData() {
        fragmentsa = new QFragment[getIndex()];
    ...
    protected void switchFragment(int switchIndex) {
        ...
        if (fragmentsa[switchIndex] == null) {
            Class cls = null;
            ...
            fragmentsa[switchIndex] = fragment;
        }
        QFragment fragment = fragmentsa[switchIndex];
        if (!fragment.isAdded()) {
        ...
        if (getIndex() != switchIndex && fragmentsa[getIndex()] != null) {
            if (tags().size() != 0 || tags().contains(fragmentsTitle()[switchIndex])) {
                transaction.remove(fragmentsa[getIndex()]);
            } else {
                transaction.hide(fragmentsa[getIndex()]);
            }
        }
    ...
    public abstract  QFragment getQFragment();
