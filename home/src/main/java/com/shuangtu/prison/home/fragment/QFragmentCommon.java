...
public abstract class QFragmentCommon extends QFragment {
    ...
    public QFragmentCommon(int clickIndex) {
    ...
    public void initData() {
        ...
        /// 通过移除不显示的Fragment
        titles = new String[]{getTitles()[clickIndex]};
        clsNames = new String[]{fragmentsTitle()[clickIndex]};

        LogUtils.d(TAG, clickIndex, titles, clsNames);

        fragments = new QFragment[clsNames.length];
    ...
    protected void switchFragment(int switchIndex) {
        ...
        if (fragments[switchIndex] == null) {
            ...
            try {
                cls = Class.forName(clsNames[switchIndex]);
                fragment = (QFragment) cls.newInstance();
            } catch (Exception e) {
                LogUtils.dTag(TAG, "创建错误", e.toString());
        ...
        if (fragment == null) {
            LogUtils.e(TAG, clsNames[switchIndex], "创建为空!!!");
            return;
        }
        if (!fragment.isAdded()) {
        ...
        if (index != switchIndex && fragments[index] != null) {
            if (tags().size() != 0 || tags().contains(clsNames[switchIndex])) {
